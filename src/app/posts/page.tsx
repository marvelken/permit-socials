...
import { Loader2, Send, MessageSquare, UserCog, AlertCircle } from "lucide-react";
import Header from "@/components/Header";
import Link from "next/link";
import { checkSocialMediaPermissions } from "../../lib/permit"; // Import the permission function
...
export default function PostsPage() {
  ...
  useEffect(() => {
    ...
    const initializePage = async () => {
      ...
      try {
        ...
        // Step 3.5: Check current user's permissions (important for creating posts)
        try {
          // Always check permission for the logged-in user, not the account being managed
          const userPermissions = await checkSocialMediaPermissions(user.id);
          console.log("User permissions for creating posts:", userPermissions);
          setPermissions(userPermissions);
        } catch (error) {
          console.error("Error fetching permissions:", error);
          // Default to no permissions if check fails
          setPermissions({
            canCreatePosts: false,
            canView: false
          });
  ...
  const createPost = async (e) => {
    ...
    if (!permissions.canCreatePosts) {
      alert("You don't have permission to create posts. Only account owners and content managers can create posts.");
      return;
    }
  ...
  // Function to render the post creation form based on permissions
  const renderPostForm = () => {
    // If user doesn't have permission to create posts, show a notification
    if (!permissions.canCreatePosts) {
      return (
        <div className="mb-8 bg-white p-4 rounded-lg shadow">
          <div className="flex items-center p-4 bg-amber-50 rounded-lg border border-amber-200">
            ...
            <p className="text-amber-700">
              You don't have permission to create posts. Only account owners and content managers can create posts.
            </p>
          </div>
        </div>
      );
    ...
    // If user has permission, show the post creation form
    return (
      <form
        onSubmit={createPost}
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        ...
        {/* Create Post Form - only show in account view, with permission check */}
        {viewMode === "account" && renderPostForm()}
