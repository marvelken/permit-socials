...
export default function PostDetail({ params }) {
  ...
  useEffect(() => {
    ...
    const checkUser = async () => {
      ...
      // Check permissions for the CURRENT USER (not the account being managed)
      // This is critical - we need to check if the current user has permission to comment
      try {
  ...
  const addComment = async (e) => {
    ...
    // Check if the current user (not the account owner) has permission to comment
    if (!permissions.canRespondToComments) {
      alert("You don't have permission to respond to comments. Only account owners and engagement specialists can reply.");
      return;
    }
  ...
  // Function to render comment form - simplified, we'll just check permissions on submission
  ...
  // const canShowReplyButton = permissions.canRespondToComments;
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
        ...
        {/* Add Comment Form - Now conditionally rendered based on permissions */}
        {renderCommentForm()}
          ...
          {post.comments.length === 0 ? (
            ...
            post.comments.map((comment) => (
              <div key={comment.id} className="bg-white rounded-lg shadow p-5">
                ...
                {/* Show reply button to everyone, but check current user's permissions when clicked */}
                <button
                  onClick={() => {
                    if (permissions.canRespondToComments) {
