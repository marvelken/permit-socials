  ...
  useEffect(() => {
    ...
    const fetchPost = async () => {
      try {
        ...
        const topLevelComments = enrichedComments.filter(comment => !comment.parent_comment_id)
        const replies = enrichedComments.filter(comment => comment.parent_comment_id)
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50">
      ...
      <main className="flex-grow container mx-auto px-4 py-8">
          ...
          <div className="flex justify-between items-start mb-4">
            <div>
              <h3 className="font-medium">
                {post.profile ? post.profile.full_name : 'Anonymous'} 
        ...
        {/* Add Comment Form */}
        <form onSubmit={addComment} className="mb-8 bg-white p-4 rounded-lg shadow">
            ...
            post.comments.map((comment) => (
              <div key={comment.id} className="bg-white rounded-lg shadow p-5">
                <div className="flex justify-between items-start mb-3">
                  <div>
                    <h3 className="font-medium">
                      {comment.profile ? comment.profile.full_name : 'Anonymous'} 
                      {!comment.profile && ` (User ID: ${comment.user_id.slice(0, 8)}...)`}
                    </h3>
                    <p className="text-sm text-gray-500">
                        ...
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <h4 className="font-medium text-sm">
                              {reply.profile ? reply.profile.full_name : 'Anonymous'} 
                              {!reply.profile && ` (User ID: ${reply.user_id.slice(0, 8)}...)`}
                            </h4>
